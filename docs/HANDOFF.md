...
npm test             # Vitest 单元测试(当前 58 通过 / 7 跳过)
```
...
1. **渲染层接弹幕 IPC**(推荐先做,可在浏览器预览验证 UI 流)
   - 播放器(`src/features/player/`)的 `DanmakuLayer`/`DanmakuSettings`/`DanmakuMatch` 目前用 `src/lib/mockData.ts` 的假弹幕。
   - 接 `window.api.danmaku`:`autoMatch`(播放时用 PlaybackSource 的 fileName/fileSize/duration)、`listProviders`、手动 `search/episodes/fetchManual`、`toAss`。
   - 走 `src/lib/dataSource.ts` 模式扩展(Electron 真实 / 浏览器 mock 回退);加 TanStack Query hooks。
   - `DanmakuMatchInput` 需要 `serverId`+`embyItemId`+`fileName`,从 `emby.playbackInfo` 返回的 `PlaybackSource.fileName/fileSize/runTimeTicks` 取。
2. **腾讯视频 provider** — segment 弹幕(docs 04 §4.5),实现 `DanmakuSourceProvider` 加入 registry(sortOrder 2)。
...
├── lib/dataSource.ts           # DataSource 抽象(Emby IPC / 浏览器 mock)— 弹幕待扩展
├── lib/queries.ts              # TanStack Query hooks(Emby 已接,弹幕待加)
├── lib/mappers.ts              # EmbyItem→MediaItem, EmbyServer→Server
├── lib/mockData.ts             # 浏览器预览 mock(含假弹幕流)
└── features/                   # 按界面分目录(player/ 含弹幕层+设置+手动匹配)
...
UI(9 屏)+ Emby 集成(真实验证)+ 弹幕逻辑层(dandanplay/bilibili 双源、匹配、缓存、ASS、多源路由)已完成且单测覆盖(58 通过)。
**下一步把弹幕能力接到播放器 UI**,以及在真实环境打通 **libmpv(Spike A)** 让弹幕真正叠加显示。
